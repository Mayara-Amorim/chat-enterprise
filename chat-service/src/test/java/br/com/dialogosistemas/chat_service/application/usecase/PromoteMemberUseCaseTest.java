...
class PromoteMemberUseCaseTest {

    @Test
    void deve_promover_membro_e_publicar_evento() {
        ...
        var useCase = new PromoteMemberUseCase(gateway, kafka);

        useCase.execute(group.getId().value(), memberUuid, ownerUuid);

        assertEquals("MEMBER_PROMOTED", kafka.events.getFirst().type());
    }

    @Test
    void deve_rejeitar_promocao_por_admin() {
        UUID ownerUuid = UUID.randomUUID();
        UUID adminUuid = UUID.randomUUID();
        UUID memberUuid = UUID.randomUUID();

        Conversation group = Conversation.createGroup(new TenantId(UUID.randomUUID()), "G", "D",
                new UserId(ownerUuid), Set.of(new UserId(ownerUuid), new UserId(adminUuid), new UserId(memberUuid)));
        ...
        assertThrows(ForbiddenOperationException.class,
                () -> useCase.execute(group.getId().value(), memberUuid, adminUuid));
    }
