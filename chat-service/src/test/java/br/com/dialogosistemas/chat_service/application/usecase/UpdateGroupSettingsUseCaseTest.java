...
class UpdateGroupSettingsUseCaseTest {
    ...
    @Test
    void deve_alterar_permissao_e_publicar_evento() {
        ...
        var useCase = new UpdateGroupSettingsUseCase(gateway, kafka);

        useCase.execute(group.getId().value(), ownerUuid, "ADMINS_ONLY");

        assertEquals(MessagingPermission.ADMINS_ONLY, group.getMessagingPermission());
        assertEquals("SETTINGS_CHANGED", kafka.events.getFirst().type());
        assertEquals("ADMINS_ONLY", kafka.events.getFirst().messagingPermission());
    }
