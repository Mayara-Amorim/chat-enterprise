...
@Service
public class DemoteMemberUseCase {

    private final ConversationGateway conversationGateway;
    private final ChatKafkaProducer kafkaProducer;

    public DemoteMemberUseCase(ConversationGateway conversationGateway, ChatKafkaProducer kafkaProducer) {
    ...
    public void execute(UUID conversationUuid, UUID targetUuid, UUID requesterUuid) {
        ...
        conversation.demoteMember(new UserId(requesterUuid), new UserId(targetUuid));
        conversationGateway.save(conversation);

        kafkaProducer.publishGroupEvent(GroupEventDTO.memberEvent(
                "MEMBER_DEMOTED", conversationUuid, targetUuid, requesterUuid));
    }
}
