...
public class AddMemberUseCase {
    ...
    public AddMemberUseCase(ConversationGateway conversationGateway, ChatKafkaProducer kafkaProducer) {
        this.conversationGateway = conversationGateway;
        this.kafkaProducer = kafkaProducer;
    }

    @Transactional
    public void execute(UUID conversationUuid, Set<UUID> newMemberUuids, UUID requesterUuid) {
        var convId = new ConversationId(conversationUuid);
        var requesterId = new UserId(requesterUuid);

        var conversation = conversationGateway.findById(convId)
                .orElseThrow(() -> new ResourceNotFoundException("Conversa nao encontrada"));

        for (UUID memberUuid : newMemberUuids) {
            var memberId = new UserId(memberUuid);
            conversation.addMember(requesterId, memberId);

            kafkaProducer.publishGroupEvent(GroupEventDTO.memberEvent(
                    "MEMBER_ADDED", conversationUuid, memberUuid, requesterUuid));
        }
