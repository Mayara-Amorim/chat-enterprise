...
public class ChatKafkaConsumer {
    ...
    @KafkaListener(topics = "chat-group-events", groupId = "chat-service-group")
    public void consumeGroupEvent(GroupEventDTO event) {
        String destination = "/topic/chat." + event.conversationId() + ".group";
        logger.info("Kafka recebeu evento de grupo: {}", event);
        websocketTemplate.convertAndSend(destination, event);
        logger.info("Evento de grupo encaminhado para WebSocket: {}", destination);
    }
}
