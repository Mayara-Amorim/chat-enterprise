...
public class RemoveMemberUseCase {
    ...
    public RemoveMemberUseCase(ConversationGateway conversationGateway, ChatKafkaProducer kafkaProducer) {
    ...
    public void execute(UUID conversationUuid, UUID targetUuid, UUID requesterUuid) {
        ...
        conversation.removeMember(new UserId(requesterUuid), new UserId(targetUuid));
        conversationGateway.save(conversation);

        kafkaProducer.publishGroupEvent(GroupEventDTO.memberEvent(
                "MEMBER_REMOVED", conversationUuid, targetUuid, requesterUuid));
    }
}
